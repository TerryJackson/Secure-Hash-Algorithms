...
// In section 1 (page 3) of the document they state that the message
// for Sha256 must be less than 2^64 bits. Well, 
// 2^64 bits = 2^61 bytes = 2^51 KB = 2^41 MB = 2^31 GB = 2^21 TB = 2,097,152 TB 
// So we are allowing inputs to be over two million terrabytes? Why not just say the
// message size in unrestricted!?
// In the padding step we add at the end the size in bits of the message. We are given 64 bits
// for this (which is why we assume the message is less than 2^64 bits) ??
// THIS IS THE PART THAT CONFUSES ME!!!
//
// For now we will just assume that message.Length <= int.MaxValue = max length of an array
//
// Also should the implementation allow messages which have their total number of bits
// not being a multiple of 8 (so that we can not represent the messages directly as an
// array of bytes). This matters because we append a 1-bit to the end of the message.

// Do we need a type for a SHA56 hash?
...
namespace SecureHashAlgorithms
{
    ...
    public static class Sha
    {
        ...
        static Sha()
        {
            ...
            /*
            //temp
            Word64[] h0 = GenerateInitialHashSha512t(256);

            Console.WriteLine("h0 length:" + h0.Length);
            Console.WriteLine("H0 length:" + H0Sha512_256.Length);


            for (int i = 0; i < 8; i++)
            {

                //if (h0[i] == H0Sha512_224[i])
                //{
        ...
        {
            byte[] paddedMessage = PadMessage512(message);
            Block512[] blocks = ConvertPaddedMessageToBlock512Array(paddedMessage);
            ...
            return ShaUtil.Word32ArrayToByteArray(H);
        }


        public static string Sha1HexString(string message)
        {
            return ShaUtil.ByteArrayToHexString(Sha1(ShaUtil.StringToByteArray(message)));
        }


        public static byte[] Sha224(byte[] message)
        {
            return Sha256Algorithm(message, H0Sha224, 224);
        }


        public static string Sha224HexString(string message)
        {
            return ShaUtil.ByteArrayToHexString(Sha224(ShaUtil.StringToByteArray(message)));
        }

        
        public static byte[] Sha256(byte[] message)
        {
            return Sha256Algorithm(message, H0Sha256, 256);
        }


        public static string Sha256HexString(string message)
        {
            return ShaUtil.ByteArrayToHexString(Sha256(ShaUtil.StringToByteArray(message)));
        }


        public static byte[] Sha512(byte[] message)
        {
            return Sha512Algorithm(message, H0Sha512, 512);
        }


        public static string Sha512HexString(string message)
        {
            return ShaUtil.ByteArrayToHexString(Sha512(ShaUtil.StringToByteArray(message)));
        }


        public static byte[] Sha384(byte[] message)
        {
            return Sha512Algorithm(message, H0Sha384, 384);
        }


        public static string Sha384HexString(string message)
        {
            return ShaUtil.ByteArrayToHexString(Sha384(ShaUtil.StringToByteArray(message)));
        }


        public static byte[] Sha512_224(byte[] message)
        {
            return Sha512Algorithm(message, H0Sha512_224, 224);
        }


        public static string Sha512_224HexString(string message)
        {
            return ShaUtil.ByteArrayToHexString(Sha512_224(ShaUtil.StringToByteArray(message)));
        }


        public static byte[] Sha512_256(byte[] message)
        {
            return Sha512Algorithm(message, H0Sha512_256, 256);
        }


        public static string Sha512_256HexString(string message)
        {
            return ShaUtil.ByteArrayToHexString(Sha512_256(ShaUtil.StringToByteArray(message)));
        }


        static byte[] Sha256Algorithm(byte[] message, Word32[] H0, int numberBits)
        {
            // We assume the numberBits truncation value is divisible by 8, so we are working with bytes.
            ...
            byte[] hash = ShaUtil.Word32ArrayToByteArray(H);
        ...
        static byte[] Sha512Algorithm(byte[] message, Word64[] H0, int numberBits)
        {
        ...
        #region Message preprossing functions

        static byte[] PadMessage512(byte[] message)
        {
            // After padding the total bits of the message will be divisible by 512.
        ...
        static Block512[] ConvertPaddedMessageToBlock512Array(byte[] M)
        {
            // We are assuming M has been padded, so the number of bits in M is divisible by 512 
            int numberBlocks = (M.Length * 8) / 512;  // same as: M.Length / 64
            ...
            for (int i = 0; i < numberBlocks; i++)
            {
                ...
                Word32[] words = ShaUtil.ByteArrayToWord32Array(B);
